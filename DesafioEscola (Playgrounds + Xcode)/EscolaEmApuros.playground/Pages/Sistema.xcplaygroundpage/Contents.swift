...
let daniel = Colaborador(nome: "Daniel", matricula: 4567, salario: 12000.00, cargo: .coordenador)
let marcelo = Colaborador(nome: "Marcelo", matricula: 4758, salario: 20000.00, cargo: .professor)
// cadastrar novos colaboradores
...
func removerColaborador(porMatricula matricula: Int){
    var posicaoRemove: Int =  -1
    for posicao in 0..<colaboradores.count{
        if colaboradores[posicao].matricula == matricula{
            posicaoRemove = posicao
        }
    }
    if posicaoRemove != -1 {
        colaboradores.remove(at: posicaoRemove)
    }
}
...
func gastosMensais()
{
    ...
    for colaborador in colaboradores {
        soma += colaborador.salario
    }
    print("Gasto mensal com colaboradores: \(soma)")
}

gastosMensais()


func gastosCargo(cargo: Cargos)->Float{
    var soma: Float = 0.0
    ...
    print("O cargo \(cargo) gasta: \(soma)")
    
    return soma
}

gastosCargo(cargo: Cargos.diretor)
...
func nomesDosColaboradores()->[String]{
    var colab: [String] = []
    for colaborador in colaboradores{
        colab.append(colaborador.nome)
    }
    colab.sort()
    return colab
}

print(nomesDosColaboradores())
